...
一个基于 **Vue 3 + Vite + Pinia + Vue Router** 的前端技术总结与面试题库项目，收录覆盖全面的前端面试题与**详尽答案解析**，支持分类浏览、全文搜索、难度筛选、收藏、深色模式，**代码块高亮 + 一键复制 / 在线运行**，**LeetCode 风格算法实战 · 在线判题（200 题三难度）**，以及 **算法错题集（浏览器永久存储 / 历史记录 / 一键生成新考试）**。
...
- 💻 **算法实战 · 在线判题**：**200 道** LeetCode 风格手写算法题，覆盖**简单 / 中等 / 困难三档难度**，分类涵盖数组 / 字符串 / 数学 / 栈队列 / 二分 / 排序 / DP / 滑动窗口 / 回溯 / 链表 / 树 / 图等，浏览器内 **Web Worker 沙箱**执行用户代码，**5 秒超时保护**，自动比对测试用例并通过/失败判定，支持题解查看、复杂度提示、答题进度统计。
- ❌ **算法错题集（新增）**：自动收录你在「算法实战」中答错的题目，**浏览器 localStorage 永久存储**，附**历史答题记录**（最近 10 次：通过/失败状态、用时、失败用例、提交代码、错误信息）与**参考题解**。支持：
  - 🎯 **从错题集生成新考试**（Fisher-Yates 洗牌，最多 20 题）
...
独立于算法实战，**自动收录**答题失败或出错的题目，存储于浏览器 **`localStorage`**（键：`algo-wrong-questions-v1`），刷新或重启浏览器后仍在：

| 能力 | 说明 |
| --- | --- |
| 🗳 **自动收录** | 在「算法实战」中运行结果为「未通过」的题目自动加入错题集 |
| ✅ **自动移除** | 同一题在后续任意一次考试中「通过」即自动移出错题集（形成错题 → 重做 → 掌握闭环） |
| 📜 **历史答题记录** | 每题保留**最近 10 次**答题记录：通过/失败、用时、失败用例编号、提交代码、执行错误 |
| 💡 **参考题解** | 错题卡片内可展开查看 Markdown 题解（思路 + 代码） |
| 🎯 **从错题生成考试** | 一键将错题集打乱（Fisher-Yates 洗牌）生成最多 20 题的新考试，重做错题 |
| 🗑 **手动移除 / 清空** | 支持单题移除（✕ 按钮）与一键清空全部错题 |
...
| Vite 5 | 构建工具 / 开发服务器 |
| Pinia | 状态管理（题目、搜索、收藏、考试、算法实战、错题集） |
| Vue Router 4 | 路由 |
...
│   │   └── wrongQuestions.js  # 错题集 store（localStorage 持久化 / 历史记录 / 生成考试）
...
│   │   ├── WrongQuestionsView.vue # 算法错题集（列表 / 历史 / 题解 / 移除 / 生成考试）
│   │   └── AboutView.vue       # 关于
│   └── data/                   # 📚 题库数据（可持续扩充）
